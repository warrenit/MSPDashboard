...
final class HaloClient
{
    ...
    private function ticketProjection(array $tickets): array
    {
        ...
        foreach ($tickets as $ticket) {
            ...
            $out[] = [
                ...
                'agent_id' => $agentId,
                'fdeleteddate' => $ticket['fdeleteddate'] ?? $ticket['fDeletedDate'] ?? null,
            ];
        }
    ...
    public function getTicketsCount(array $queryParams = []): int
    {
        $token = $this->getAccessToken();
        $path = (string) $this->settings->get('halo_tickets_path', '/tickets');

        $params = array_merge($queryParams, ['page_size' => 1, 'page_no' => 1]);
        $res = $this->request('GET', $path, $params, $token);
        if ($res['ok'] && is_array($res['json'])) {
            $json = $res['json'];
            foreach (['record_count', 'recordCount', 'total', 'total_count'] as $key) {
        ...
        // fallback: full paging count
        return $this->countByPaging($token, $path, $queryParams);
    }

    public function getScopedOpenAndUnassignedCounts(bool &$missingDeletedField, array &$sampleOpenIds, array &$sampleUnassignedIds): array
    {
        $missingDeletedField = false;
        $sampleOpenIds = [];
        $sampleUnassignedIds = [];

        $queryParams = [
            'status_id_not' => 9,
            'tickettype_ids' => '1,21,28,40',
            'fdeleteddate_is_null' => '1',
        ];

        $totalOpenFromCount = $this->getTicketsCount($queryParams);

        $token = $this->getAccessToken();
        $path = (string) $this->settings->get('halo_tickets_path', '/tickets');
        $pageSize = max(50, min(300, (int) $this->settings->get('halo_page_size', '200')));
        $maxPages = max(1, min(50, (int) $this->settings->get('halo_max_pages', '10')));
        ...
        for ($page = 1; $page <= $maxPages; $page++) {
            ...
            foreach ($rows as $ticket) {
                ...
                $ticketTypeId = $this->toIntOrNull($ticket['tickettype_id'] ?? $ticket['tickettypeid'] ?? null);
                $agentId = $this->toIntOrNull($ticket['agent_id'] ?? $ticket['agentid'] ?? $ticket['assigned_agent_id'] ?? null);
                $ticketId = $this->toIntOrNull($ticket['id'] ?? null);

                $fdeletedRaw = $ticket['fdeleteddate'] ?? $ticket['fDeletedDate'] ?? null;
                $hasDeletedField = array_key_exists('fdeleteddate', $ticket) || array_key_exists('fDeletedDate', $ticket);
                if (!$hasDeletedField) {
                    $missingDeletedField = true;
                }
                $isDeleted = is_string($fdeletedRaw) ? trim($fdeletedRaw) !== '' : ($fdeletedRaw !== null);

                $inScope = $statusId !== null && $ticketTypeId !== null && $statusId !== 9 && in_array($ticketTypeId, [1, 21, 28, 40], true) && !$isDeleted;
                if (!$inScope) {
                    continue;
                }

                $totalOpenLocal++;
                if ($ticketId !== null && count($sampleOpenIds) < 5) {
                    $sampleOpenIds[] = $ticketId;
                }

                if ($agentId === 1) {
                    $unassignedLocal++;
                    $bucketAgent0++;
                    if ($ticketId !== null && count($sampleUnassignedIds) < 5) {
                        $sampleUnassignedIds[] = $ticketId;
                    }
                } elseif ($agentId !== null && $agentId > 1) {
                    $bucketAgentGt0++;
                    if ($ticketId !== null && count($sampleAssignedIds) < 5) {
                        $sampleAssignedIds[] = $ticketId;
                    }
                }
            }
        ...
        // prefer local exact if available, otherwise record_count attempt
        $totalOpen = $totalOpenLocal > 0 || $sampleOpenIds !== [] ? $totalOpenLocal : $totalOpenFromCount;
        ...
        return [
            ...
            'filters' => $queryParams,
            'agentAssignedBucket' => [
                'agent_id_0' => $bucketAgent0,
                'agent_id_gt0' => $bucketAgentGt0,
            ],
            'sampleAssignedIds' => $sampleAssignedIds,
        ];
    }

    private function countByPaging(string $token, string $path, array $queryParams): int
    {
        $pageSize = 200;
        $count = 0;
        for ($page = 1; $page <= 50; $page++) {
            $params = array_merge($queryParams, ['page_no' => $page, 'page_size' => $pageSize]);
            $res = $this->request('GET', $path, $params, $token);
            if (!$res['ok']) {
                break;
            }
