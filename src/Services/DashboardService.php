...
final class DashboardService
{
    ...
    public function payload(): array
    {
        ...
        $debug = null;
        if ($this->haloClient->isEnabled() && $this->haloClient->hasCredentials()) {
            try {
                $missingDeletedField = false;
                $sampleOpen = [];
                $sampleUnassigned = [];
                $countData = $this->haloClient->getScopedOpenAndUnassignedCounts($missingDeletedField, $sampleOpen, $sampleUnassigned);
                $sampleAssigned = is_array($countData['sampleAssignedIds'] ?? null) ? $countData['sampleAssignedIds'] : [];

                $tiles['totalOpenCount'] = (int) ($countData['totalOpen'] ?? 0);
                $tiles['unassignedCount'] = (int) ($countData['unassigned'] ?? 0);

                if ($missingDeletedField && in_array($halo['status']['state'] ?? 'grey', ['green', 'amber'], true)) {
                    $halo['status']['state'] = 'amber';
                    $halo['status']['message'] = 'fdeleteddate missing from Halo response';
                }

                if (isset($_GET['debug']) && $_GET['debug'] === '1') {
                    $debug = [
                        'haloFilters' => $countData['filters'] ?? [],
                        'totalOpenCount' => $tiles['totalOpenCount'],
                        'unassignedCount' => $tiles['unassignedCount'],
                        'sampleOpenIds' => $sampleOpen,
                        'sampleUnassignedIds' => $sampleUnassigned,
                        'sampleAssigned' => $sampleAssigned,
                        'agentAssignedBucket' => is_array($countData['agentAssignedBucket'] ?? null) ? $countData['agentAssignedBucket'] : ['agent_id_0' => 0, 'agent_id_gt0' => 0],
                    ];
                }
            } catch (\Throwable $e) {
                if (in_array($halo['status']['state'] ?? 'grey', ['green', 'amber'], true)) {
                    $halo['status']['state'] = 'amber';
                    $halo['status']['message'] = 'Halo count fallback error';
                }
