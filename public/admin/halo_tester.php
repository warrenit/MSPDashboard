...
$tmpDir = Config::configDir() . '/tmp';
if (!is_dir($tmpDir)) {
    @mkdir($tmpDir, 0700, true);
}

// best-effort cleanup > 1 day
if (is_dir($tmpDir)) {
    foreach (glob($tmpDir . '/halo_tester_*.json') ?: [] as $file) {
        if (is_file($file) && filemtime($file) !== false && filemtime($file) < time() - 86400) {
            @unlink($file);
        }
    }
}
...
if (isset($_GET['download']) && is_string($_GET['download'])) {
    ...
    if (!preg_match('/^halo_tester_\d{8}_\d{6}_[a-f0-9]{8}\.json$/', $name)) {
...
if (isset($_GET['run']) && $_GET['run'] === '1') {
    try {
        ...
        $rawFull = (string) ($result['preview'] ?? '');
        if (trim($rawFull) === '') {
            $rawFull = '{}';
        }

        $decodedCandidate = json_decode($rawFull, true);
        if (is_array($decodedCandidate)) {
            $decoded = $decodedCandidate;
            $pretty = json_encode($decodedCandidate, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
            if (is_string($pretty)) {
                $rawFull = $pretty;
            }
        ...
        $rawPreview = mb_substr($rawFull, 0, 20000);

        if (is_dir($tmpDir)) {
            $downloadName = 'halo_tester_' . gmdate('Ymd_His') . '_' . bin2hex(random_bytes(4)) . '.json';
            $downloadPath = $tmpDir . '/' . $downloadName;
            @file_put_contents($downloadPath, $rawFull, LOCK_EX);
...
<div class="container">
    ...
    <?php if (is_array($result)): ?>
        ...
        <?php if ($downloadName !== ''): ?>
            <p><a href="?download=<?= urlencode($downloadName) ?>">Download full JSON</a></p>
        <?php endif; ?>
            ...
            <div style="overflow:auto; border:1px solid #30405e; border-radius:8px; margin-bottom:10px;">
                <table style="width:100%; border-collapse:collapse; font-size:0.9rem;">
                    ...
                    <tr style="background:#0b1424;">
                        ...
                        <th style="text-align:left;padding:8px;border-bottom:1px solid #30405e;">fdeleteddate</th>
                    ...
                    <tbody>
                    <?php foreach (array_slice($tickets, 0, 50) as $ticket): if (!is_array($ticket)) { continue; } ?>
                        <tr>
                            ...
                            <td style="padding:8px;border-bottom:1px solid #243551;"><?= htmlspecialchars(tval($ticket, ['fdeleteddate', 'fDeletedDate']), ENT_QUOTES, 'UTF-8') ?></td>
        ...
        <details>
            ...
            <textarea style="width:100%;height:420px;overflow:auto;background:#0b1424;color:#f4f7ff;border:1px solid #30405e;border-radius:8px;padding:10px;" readonly><?= htmlspecialchars($rawFull !== '' ? $rawFull : $rawPreview, ENT_QUOTES, 'UTF-8') ?></textarea>
        </details>
    <?php endif; ?>
